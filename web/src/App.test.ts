...
describe('App shell', () => {
  it('renders the product title, locale, loading completion, and module list', async () => {
    ...
    await vi.waitFor(() => expect(target.textContent).toContain('bundled-source'));
  ...
  it('shows a loading state until the bootstrap transport resolves', async () => {
    ...
    const app = mount(App, {
      target,
      props: {
        transport: new MockTransport({ bootstrapPromise: pending.promise, modules }),
      },
    });
  ...
  it('renders a structured error state from the transport', async () => {
    ...
    await vi.waitFor(() => expect(target.textContent).toContain('Unable to load MyFitAnalytics'));

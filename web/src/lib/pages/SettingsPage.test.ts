...
describe('SettingsPage', () => {
  ...
  it('treats workspace and package dialog cancellation as a no-op', async () => {
    ...
    await vi.waitFor(() => expect(target.textContent).toContain('Choose Workspace'));
    target.querySelector<HTMLButtonElement>('[data-action="choose-workspace"]')?.click();
    target.querySelector<HTMLButtonElement>('[data-action="install-package"]')?.click();
    await vi.waitFor(() => expect(target.textContent).not.toContain('Unable to'));
  ...
  it('refreshes catalog after lifecycle actions and keeps provider controls explicit', async () => {
    ...
    await vi.waitFor(() => expect(target.textContent).toContain('Hevy'));
    target.querySelector<HTMLButtonElement>('[data-module-id="hevy"][data-action="disable"]')?.click();
    await vi.waitFor(() => expect(transport.calls).toContain('setModuleEnabled:hevy:false'));
    target.querySelector<HTMLButtonElement>('[data-module-id="hevy"][data-action="provider"]')?.click();
    await vi.waitFor(() => expect(transport.calls).toContain('selectProvider:body.weight:hevy'));
    await vi.waitFor(() => expect(target.textContent).toContain('Active provider'));
    target.querySelector<HTMLButtonElement>('[data-module-id="hevy"][data-action="choose-inbox"]')?.click();
    await vi.waitFor(() => expect(transport.calls).toContain('chooseSourceInbox:hevy'));
    unmount(app);
  });
  ...
  it('renders a localized lifecycle failure without exposing a path field', async () => {
    ...
    await vi.waitFor(() => expect(target.textContent).toContain('Configure a workspace before enabling this source.'));
  ...
  it('requires confirmation before uninstalling a disabled module', async () => {
    ...
    target
      .querySelector<HTMLButtonElement>('[data-module-id="summary-dashboard"][data-action="uninstall"]')
      ?.click();
    await vi.waitFor(() => expect(target.textContent).toContain('Are you sure you want to uninstall'));
    expect(transport.calls).not.toContain('uninstallModule:summary-dashboard');
    target.querySelector<HTMLButtonElement>('[data-action="confirm-uninstall"]')?.click();
    await vi.waitFor(() => expect(transport.calls).toContain('uninstallModule:summary-dashboard'));
    unmount(app);
  });
});
